...
／／以上可看可不看，正文在下面
用terminal一路cd进入BBox-Label-Tool-master， 运行python main.py进行label，注意所有图片要放在images／001文件夹中，打开标图软件后在imageDir里输入1， 标记完一张图后点击下方next

用以上方式标完所有图片后，在terminal中运行python together.py, 所有 label都在label.txt中
注意！！！！所有图片必须是.jpg格式！！！！！！
